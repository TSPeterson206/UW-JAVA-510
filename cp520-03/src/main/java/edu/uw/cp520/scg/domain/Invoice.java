...
public class Invoice {
    ...
    public void extractLineItems(TimeCard timeCard) {
        ...
        for (ConsultantTime item : timeCardList) {

            if (item.getDate().getMonth() == invoiceMonth
                && item.getDate().getYear() == invoiceYear) {
                if (dateStopper == true) {
                    startingDay = item.getDate();
                    invoiceYear = item.getDate().getYear();
                    dateStopper = false;
                }
                lineItem = new InvoiceLineItem(item.getDate(),
                    timeCard.getConsultant(), item.getSkill(), item.getHours());
