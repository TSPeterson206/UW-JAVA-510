...
// Need to create function for individual entries with details. takes in formatter,time array, and boolean to determine billing or no.

        fm.format(NON_BILLABLE_TIME_HEADER_FORMAT).format(LINE_HEADER_FORMAT);

        // Need to create function for individual entries with details
    ...
    public String toString() {
        return String.format("TimeCard for: %s, Week Starting: %2$tb",
            consultant.getName(), Locale.US);
    };
}
