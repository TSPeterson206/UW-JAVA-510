...
public final class TimeCardConsultantComparatorTest {
    ...
    public void testCompare() {
        ...
        final TimeCard firstTimeCard = new TimeCard(
            new Consultant(new PersonalName(CODER, JANE, Q_DOT)), date);
        TimeCard secondTimeCard = new TimeCard(
            new Consultant(new PersonalName(CODER, JANE, Q_DOT)), date);

        assertTrue(timeCardConsultantComparator.compare(firstTimeCard,
            secondTimeCard) == 0);

        secondTimeCard = new TimeCard(
            new Consultant(new PersonalName(CODER, JOHN, Q_DOT)), date);
        assertTrue(timeCardConsultantComparator.compare(firstTimeCard,
            secondTimeCard) < 0);
        assertTrue(timeCardConsultantComparator.compare(secondTimeCard,
            firstTimeCard) > 0);
    }
}
